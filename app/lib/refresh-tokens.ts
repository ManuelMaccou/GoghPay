  ...
  if (shouldRefresh && daysUntilExpiration > 0) {
    ...
    try {
      ...
      if (error instanceof Error) {
        console.error(`Failed to refresh token for merchant ${square_merchant_id}: ${error.message}`);
        await AdminError.create({
          merchantId: square_merchant_id,
          attemptedTask: 'Check refresh token',
        ...
        console.error(`Failed to refresh token for merchant ${square_merchant_id}: ${String(error)}`);
        await AdminError.create({
          merchantId: square_merchant_id,
