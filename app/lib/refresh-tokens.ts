...
export const checkAndRefreshToken = async (merchantId: string): Promise<boolean> => {
  try {
    ...
    // HERE, ADD A 'REVOKED' FIELD TO THE DB. ADD 'TRUE' IF THE MERCHANT REVOKES
    // IF REVOKED IS 'TRUE', REGARDSLESS OF THESE VALUES MISSING, PASS A MESSAGE BACK
    // TO THE CLIENT SO YOU CAN SHOW THE CURRENT ERROR MESSAGE THAT IS THERE RIGHT NOW. ABOUT THE
    // SELLER RECONNECTING.
    // IF 'REVOKED' IS NOT TRUE, AND THESE VALUES ARE MISSING, THEN RERUN THE AUTH IF THE REFRESH TOKEN IS THERE
    // IF 'REVOKES IS NOT TRUE AND NEITHER REFRESH TOKEN NOR ACCESS TOKEN ARE THERE, HAVE THE MERCHANT RECONNECT
    // IF THE INTEGRATION PAGE LOADS AND ANY VALUE IS NOT THERE, GET A NEW TOKEN IF THE REFRESH IS THERE
    // IF THE REFRESH TOKEN IS NOT THERE, TRY TO AUTO-REVOKE. IF YOU CAN'T, SEE IF YOU CAN JUST SHOW THE CONNECT BUTTON AGAIN.
    // SEE WHAT HAPPENS IF YOU ARE ARE AUTHED, BUT DELETE THE REFRESH TO SIMULATE MISSING A REFRESH TOKEN. THEN TRY
    // CHANGING THE BUTTON TO CONNECT AND SEE WHAT HAPPENS. IT SHOULD RESAVE THE VALUES TO YOUR DB. THAT MIGHT BE THE SHORT CUT.
    // IF THE INTEGRATION PAGE LOADS, AND ANY FIELD IS MISSING, CHANGE THE BUTTON AND RECONNECT. THAT MIGHT BE SOLUTION
    ...
    if (shouldRefresh && daysUntilExpiration > 0) {
      ...
      try {
        ...
        merchant.square_merchant_id = data.merchant_id;
        merchant.square_refresh_token = encrypt(data.refresh_token);
        await merchant.save();
